...
export default function ChatRoom({ roomId, roomName = roomId, userData }) {
  ...
  const senderName = userData?.login ? userData.fullName : "Anonymous";
  const senderEmail = userData?.login ? userData.email : "anonymous";
  ...
  async function loadMessages() {
    ...
    try {
      const res = await fetch(`/api/chat/rooms/${encodeURIComponent(roomId)}/msg?limit=50`, { cache: "no-store" });
    ...
    <section style={{ border: "1px solid #ddd", borderRadius: 12, overflow: "hidden" }}>
      ...
      <form onSubmit={sendMessage} style={{ display: "flex", gap: 8, padding: 12, borderTop: "1px solid #eee" }}>
        <input
          ...
          placeholder={userData?.login ? "메시지를 입력하세요" : "로그인 후 작성할 수 있습니다"}
          disabled={!userData?.login}
          style={{ flex: 1, padding: 12, border: "1px solid #ddd", borderRadius: 8 }}
        />
        <button
          type="submit"
          disabled={!userData?.login || sending || !text.trim()}
